...
Purpose: Connecting to the ESP32 via Blutooth and saving quaternion data to a csv file
'''

import asyncio
import csv
from datetime import date
from bleak import BleakClient, BleakSscanner
import uuid
import sys
...
def create_new_csv_file(body_part) -> str:
    ...
    name = f"{body_part}-qWqXqYqZ-data-{date.today().strftime('%Y-%m-%d')}.csv"
    field_names = ["wX", "wY", "wZ"]
